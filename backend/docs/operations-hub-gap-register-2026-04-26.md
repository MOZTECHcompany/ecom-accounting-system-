...
- Google Ads 權威 mapping：`8052579705 => MOZTECH_TW / market TW`、`8602556100 => BONSON / market TW`、`8672054842 => MORITEK / market TW`、`5801010919 => MOZTECH_US / market US / 美國獨立站`。`5801010919` 的 timezone 是 America/New_York，不得混入台灣 MOZTECH；實際歸屬仍只看 `GOOGLE_ADS_ACCOUNTS_JSON`，不以名稱或 timezone 推論。
- 2026-08-03 帳號 scope 再核對：Google `6171193760 萬魔未來工學院` 已納入 dormant portfolio，近 30 日零花費，未來有花費 campaign 必須先有精確品牌 mapping；`6215621647` 僅是 manager account。Meta `act_791999392715510 MORITEK TW` 已納入 MORITEK；`act_140675171327599` 為 portfolio，現有三個有花費 campaign 精確對應 AIRITY。Meta token 另可見 `act_1047400912417236 DAILY LAB OFFICIAL`，但它不在公司權威品牌主檔，維持「可見但未納入」，不得用名稱自動歸屬。
- 2026-08-03 campaign-granular 正式基線已完成：Meta `215` 筆 + Google `162` 筆，共 `377` 筆，legacy account/day aggregate 為 `0`。禁止把 production traffic 退回 `ecom-accounting-backend-brand-reg-v2` 之前。
...
- 2026-08-03 已移除廣告品牌歸屬中的硬編碼帳號特例與依帳號 / campaign / 描述文字模糊猜品牌。單一品牌帳號只採 `META_ADS_ACCOUNTS_JSON` / `GOOGLE_ADS_ACCOUNTS_JSON` 的明確 `reportBrand` 或 `brand`；未映射帳號固定隔離為 `待對應`，並回傳 `mappingStatus`、diagnostic、account-level coverage 與 `releaseReady=false`。
- 多品牌共用帳號新增 `brandMode=portfolio` / `allowedBrands` 與精確 campaign ID 主檔。可把規則內嵌在 `campaignBrandMappings`，或放進 `META_ADS_CAMPAIGN_BRANDS_JSON` / `GOOGLE_ADS_CAMPAIGN_BRANDS_JSON`；portfolio 未列出的 campaign 不得靠名稱猜測，仍為 `待對應`。目前 production `act_140675171327599` 必須維持 portfolio，不可永久硬設整個帳號為 AIRITY。
- `GET /integrations/meta-ads/readiness` 與 Google Ads readiness 現在用 campaign-level live probe 驗證 mapping coverage；insights、sync 與 `GET /reports/ad-performance-summary` 同步提供 coverage / diagnostics，讓連線健康與可安全分析分開判斷。這次只完成程式與測試，Cloud Run 環境主檔更新及正式部署仍須由發佈流程另行驗證。
- Meta / Google 新同步列一律採 campaign granularity，`sourceId` 固定為 `accountRef:campaignId:YYYY-MM-DD`。同一帳號 / 日期的新 campaign 列全部寫入成功後，connector 會刪除舊的 `accountRef:YYYY-MM-DD` 衍生彙總 Expense（明細為 cascade），避免新舊資料重複計費；報表只從結構化 `sourceId` 取 account / campaign，再查權威 registry，不讀 description 或 campaign 名稱猜品牌。
- LUNA worker 已加入第一方 Dashboard 發布完成條件：本機 HTML 會先重新驗證，再用 GCE metadata 的短效 Google ID token 發到專用 publication service；服務回傳的 URL、SHA、bytes、entity、source digest、report date、task、雙平台 coverage 與品牌 mapping coverage 都需完全相符，且 `https://ads.corely.cc/d/...` 回讀 bytes / SHA / 安全標頭一致後才可寫入 task result。Discord durable outbox 會把該 URL 與同一份 HTML 附件交叉綁定，重試不可換 URL 或 artifact；production 設 `LUNA_ADS_DASHBOARD_PUBLICATION_REQUIRED=true` 後，publisher 缺失或驗證失敗會阻止 task 標記完成。針對發布與 Discord 的本機測試已通過；GCP service、Cloud SQL/GCS、DNS/TLS 與正式 Discord 驗收仍屬部署階段，不能以本機測試宣稱已上線。
