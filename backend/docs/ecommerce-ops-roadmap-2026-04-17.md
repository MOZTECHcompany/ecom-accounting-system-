...
- 超收明細已支援後端分頁、分類篩選與 CSV 審核清單匯出，讓 600+ 筆正式候選可以逐頁處理，不再只顯示第一批資料。
- 會計工作台「串接準備」分頁已支援複製待補清單與匯出 CSV，讓使用者可以把外部 API 權限、報表、密鑰與格式需求直接整理給財務、平台或工程窗口，不必只看 repo 文件。
- 銀行交易明細頁已補上銀行對帳單 CSV 範本下載與實際匯入流程；匯入前必須選銀行帳戶，前端會送 `{ csvContent }` 到 `/banking/accounts/:id/import-statement`，可作為後續銀行流水 matching 的第一個操作入口。
- 銀行匯入底層已補強 CSV 解析，支援引號欄位、逗號 / 分號 / tab、民國年日期與常見銀行金額格式，降低正式銀行匯出檔一進來就解析錯的風險。
- 銀行匯入操作已加上只讀預覽步驟，會先顯示可匯入筆數、略過列與樣本交易，按下「確認匯入」後才新增銀行交易並跑初步對帳。
- 產品與庫存頁已補上 SKU / 初始庫存匯入範本下載與 dry-run 預覽，會先顯示可匯入列數、新建倉庫、新建產品、更新產品與庫存行數，確認後才正式寫入產品與庫存。
