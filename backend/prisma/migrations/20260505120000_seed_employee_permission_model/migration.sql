-- Prisma's uuid() default is generated by the client and is not a database
-- column default. Raw SQL migrations must therefore provide primary keys.
INSERT INTO "permissions" ("id", "resource", "action", "description")
VALUES
  (gen_random_uuid()::text, 'access_control', 'read', '查看帳號與權限管理'),
  (gen_random_uuid()::text, 'access_control', 'update', '維護帳號、角色與權限設定'),
  (gen_random_uuid()::text, 'purchase_orders', 'read', '查看採購訂單'),
  (gen_random_uuid()::text, 'purchase_orders', 'create', '建立採購訂單'),
  (gen_random_uuid()::text, 'inventory', 'read', '查看庫存與產品'),
  (gen_random_uuid()::text, 'inventory', 'update', '維護庫存與產品'),
  (gen_random_uuid()::text, 'banking', 'read', '查看銀行與對帳資料'),
  (gen_random_uuid()::text, 'banking', 'update', '執行銀行與對帳作業'),
  (gen_random_uuid()::text, 'reports', 'read', '查看報表中心'),
  (gen_random_uuid()::text, 'attendance_self', 'read', '查看自己的打卡與出勤'),
  (gen_random_uuid()::text, 'leave_self', 'read', '查看與申請自己的請假'),
  (gen_random_uuid()::text, 'payroll_self', 'read', '查看自己的薪資單'),
  (gen_random_uuid()::text, 'payroll_self_breakdown', 'read', '查看自己的薪資明細與計算方式'),
  (gen_random_uuid()::text, 'profile_self', 'read', '查看自己的個人資料'),
  (gen_random_uuid()::text, 'employees_admin', 'read', '查看員工與部門資料'),
  (gen_random_uuid()::text, 'employees_admin', 'update', '維護員工與部門資料'),
  (gen_random_uuid()::text, 'attendance_admin', 'read', '查看考勤後臺審核資料'),
  (gen_random_uuid()::text, 'attendance_admin', 'update', '維護考勤規則與執行審核'),
  (gen_random_uuid()::text, 'payroll_admin', 'read', '查看薪資批次與設定'),
  (gen_random_uuid()::text, 'payroll_admin', 'update', '執行薪資計算與發薪')
ON CONFLICT ("resource", "action") DO UPDATE
SET "description" = EXCLUDED."description";

INSERT INTO "roles" ("id", "code", "name", "description", "hierarchy_level")
VALUES
  (gen_random_uuid()::text, 'EMPLOYEE', 'EMPLOYEE', '一般員工，預設只可查看自己的資料與進行自助作業', 5)
