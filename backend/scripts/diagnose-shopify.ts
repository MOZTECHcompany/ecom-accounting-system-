...
async function diagnose() {
  const logger = new Logger('ShopifyDiagnosis');
...
function request(hostname: string, path: string, token: string): Promise<any> {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode && (res.statusCode < 200 || res.statusCode >= 300)) {
          const error: any = new Error(`Status Code: ${res.statusCode}`);
