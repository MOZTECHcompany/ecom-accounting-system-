  ...
  const handleClockIn = async () => {
    try {
      ...
      if (backendMessage.includes("Employee record not found")) {
        ...
        message.error(backendMessage || "打卡失敗");
      }
    } finally {
      setLoading(false);
    }
            ...
            {todayRecords.length > 0 ? (
              <div className="space-y-3">
                {todayRecords.map((record, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 rounded-xl bg-white/40 border border-white/40 hover:bg-white/60 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
