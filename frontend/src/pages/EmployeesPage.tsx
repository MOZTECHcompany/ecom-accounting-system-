...
const EmployeesTab = ({ departments }: { departments: Department[] }) => {
  ...
  const columns = [
    ...
    { 
      ...
      key: 'department',
      render: (deptId: string) => departments.find(d => d.id === deptId)?.name || '-'
    },
    { 
      title: '到職日', 
      dataIndex: 'hireDate', 
    ...
    },
    {
      ...
      render: (isActive: boolean) => (
        <Tag color={isActive ? 'green' : 'red'}>{isActive ? '在職' : '離職'}</Tag>
      ),
    },
    {
      title: '操作',
  ...
  return (
    <div className="glass-card p-6">
      <div className="flex justify-between items-center mb-6">
        <Title level={4} className="!mb-0 !font-light">員工名單</Title>
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="employeeNo" label="員工編號" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item name="hireDate" label="到職日" rules={[{ required: true }]}>
            <DatePicker className="w-full" />
          </Form.Item>
          <Form.Item name="salaryBaseOriginal" label="本薪" rules={[{ required: true }]}>
...
const DepartmentsTab = ({ departments, reload }: { departments: Department[], reload: () => void }) => {
  const [createOpen, setCreateOpen] = useState(false)
  const [form] = Form.useForm()
  ...
  return (
    <div className="glass-card p-6">
      <div className="flex justify-between items-center mb-6">
        <Title level={4} className="!mb-0 !font-light">部門管理</Title>
...
const EmployeesPage: React.FC = () => {
  ...
  return (
    ...
    >
      <div>
        <Title level={2} className="!mb-1 !font-light">員工與部門管理</Title>
      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            children: <DepartmentsTab departments={departments} reload={fetchDepartments} />,
          },
        ]}
      />
    </motion.div>
